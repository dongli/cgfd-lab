...
contains
  ...
  subroutine beam_warming(rho)
    ...
    do i = 1, nx
      if (u >= 0) then
        flux(i+1) = 0.5d0 * (u * (3 * rho(i  ) - rho(i-1)) - dt / dx * u**2 * (rho(i  ) - rho(i-1)))
      else
        flux(i+1) = 0.5d0 * (u * (3 * rho(i+1) - rho(i+2)) - dt / dx * u**2 * (rho(i+2) - rho(i+1)))
      end if
    end do
  ...
  subroutine output(rho)
    ...
    write(file_name, "('beam_warming.', I3.3, '.nc')") time_step
