...
contains
  ...
  subroutine run_swm(old, new)
    ...
    ! uv________uv________uv________uv     j+1
    ! |         |         |         |
    ! |         |         |         |
    ! |    h    |    h    |    h    |  j+1
    ! |         |         |         |
    ! uv________uv________uv________uv      j
    ! |         |         |         |
    ! |         |         |         |
    ! |    h    |    h    |    h    |   j
    ! |         |         |         |
    ...
    ! |    h    |    h    |    h    |  j-1
    ! |         |         |         |
    ! uv________uv________uv________uv
    !
    !     i-1        i        i+1               <- full x grids
    !          i-1        i        i+1          <- half x grids
    !                                   ^   ^
    !                                   |   |
    !                          full y grids |
    !                                  half y grids
    ...
    do j = 1, ny
      do i = 1, nx
        du(i,j,old) = &
         ...
         -g * (h(i+1,j,old) + h(i+1,j+1,old) - h(i,j,old) - h(i,j+1,old)) / dx / 2.0_rp
        dv(i,j,old) = &
          ! Coriolis force
         -f * u(i,j,old) &
          ! Pressure gradient force
         -g * (h(i,j+1,old) + h(i+1,j+1,old) - h(i,j,old) - h(i+1,j,old)) / dy / 2.0_rp
        dh(i,j,old) = &
          ! Mass flux gradient along x direction
         -h0 * (u(i,j,old) + u(i,j-1,old) - u(i-1,j,old) - u(i-1,j-1,old)) / dx / 2.0_rp &
          ! Mass flux gradient along y direction
         -h0 * (v(i,j,old) + v(i-1,j,old) - v(i,j-1,old) - v(i-1,j-1,old)) / dy / 2.0_rp
      end do
    end do
  ...
  subroutine output(k)
    ...
    write(file_name, "('linear_swm_b_grid.', I3.3, '.nc')") time_step
    ...
    ierr = nf90_def_var(ncid, 'u', nf90_double, [xi_dimid,yi_dimid,time_dimid], u_varid)

    ierr = nf90_def_var(ncid, 'v', nf90_double, [xi_dimid,yi_dimid,time_dimid], v_varid)
