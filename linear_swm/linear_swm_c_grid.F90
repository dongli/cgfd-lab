...
contains
  ...
  subroutine run_swm(old, new)
    ...
    !  ____v_________v_________v____       j+1
    ! |         |         |         |
    ! |         |         |         |
    ! u    h    u    h    u    h    u  j+1
    ! |         |         |         |
    ! |____v____|____v____|____v____|       j
    ! |         |         |         |
    ! |         |         |         |
    ! u    h    u    h    u    h    u   j
    ! |         |         |         |
    ! |____v____|____v____|____v____|      j-1
    ! |         |         |         |
    ! |         |         |         |
    ! u    h    u    h    u    h    u  j-1
    ! |         |         |         |
    ...
    do j = 1, ny
      do i = 1, nx
        du(i,j,old) = &
          ! Coriolis force
          f * (v(i,j-1,old) + v(i+1,j-1,old) + v(i,j,old) + v(i+1,j,old)) * 0.25_rp &
          ! Pressure gradient force
         -g * (h(i+1,j,old) - h(i,j,old)) / dx
        dv(i,j,old) = &
          ! Coriolis force
         -f * (u(i,j+1,old) + u(i-1,j+1,old) + u(i,j,old) + u(i-1,j,old)) * 0.25_rp &
          ! Pressure gradient force
         -g * (h(i,j+1,old) - h(i,j,old)) / dy
        dh(i,j,old) = &
          ! Mass flux gradient along x direction
         -h0 * (u(i,j,old) - u(i-1,j,old)) / dx &
          ! Mass flux gradient along y direction
         -h0 * (v(i,j,old) - v(i,j-1,old)) / dy
      end do
    end do
  ...
  subroutine output(k)
    ...
    write(file_name, "('linear_swm_c_grid.', I3.3, '.nc')") time_step
    ...
    ierr = nf90_def_var(ncid, 'u', nf90_double, [xi_dimid,y_dimid,time_dimid], u_varid)

    ierr = nf90_def_var(ncid, 'v', nf90_double, [x_dimid,yi_dimid,time_dimid], v_varid)
