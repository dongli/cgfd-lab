...
! boundary condition for Lax-Wendroff finite difference scheme.
!
! Li Dong <dongli@lasg.iap.ac.cn>
!
! - 2018-03-20: Initial creation.

program lax_wendroff_adv_1d_case
...
contains
  ...
  subroutine lax_wendroff(rho)
    ...
    do i = 1, nx
      flux(i+1) = 0.5d0 * (u * (rho(i+1) + rho(i)) - dt / dx * u**2 * (rho(i+1) - rho(i)))
    end do
  ...
  subroutine output(rho)
    ...
    write(file_name, "('lax_wendroff.', I3.3, '.nc')") time_step
...
end program lax_wendroff_adv_1d_case
